...
DO $$ BEGIN
 CREATE TYPE "enum_newsletters_status" AS ENUM('draft', 'scheduled', 'sent');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
 CREATE TYPE "enum_postmark_templates_template_type" AS ENUM('Standard', 'Layout');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS "newsletters" (
	...
	"status" "enum_newsletters_status" NOT NULL,
...
CREATE TABLE IF NOT EXISTS "postmark_templates" (
	...
	"associated_server_id" numeric NOT NULL,
	"active" boolean,
	"alias" varchar,
	"templateType" "enum_postmark_templates_template_type" NOT NULL,
...
CREATE INDEX IF NOT EXISTS "plans_features_order_idx" ON "plans_features" ("_order");
CREATE INDEX IF NOT EXISTS "plans_features_parent_id_idx" ON "plans_features" ("_parent_id");
...
DO $$ BEGIN
 ALTER TABLE "plans_features" ADD CONSTRAINT "plans_features__parent_id_plans_id_fk" FOREIGN KEY ("_parent_id") REFERENCES "plans"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
...
DO $$ BEGIN
 ALTER TABLE "newsletters_rels" ADD CONSTRAINT "newsletters_rels_media_id_media_id_fk" FOREIGN KEY ("media_id") REFERENCES "media"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
