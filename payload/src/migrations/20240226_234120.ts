...
DO $$ BEGIN
 CREATE TYPE "enum_contacts_subscription_management_email_status" AS ENUM('Active', 'Unsubscribed');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;

DO $$ BEGIN
 CREATE TYPE "enum_contacts_subscription_management_email_permission_status" AS ENUM('Express', 'Implied');
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
...
CREATE TABLE IF NOT EXISTS "contacts" (
	"id" serial PRIMARY KEY NOT NULL,
	"personal_info_email_address" varchar NOT NULL,
	"personal_info_first_name" varchar,
	"personal_info_last_name" varchar,
	"subscription_management_email_status" "enum_contacts_subscription_management_email_status",
	"subscription_management_email_permission_status" "enum_contacts_subscription_management_email_permission_status",
	"subscription_management_confirmed_opt_in_date" timestamp(3) with time zone,
	"subscription_management_confirmed_opt_in_source" varchar,
	"subscription_management_confirmed_opt_out_date" timestamp(3) with time zone,
	"subscription_management_confirmed_opt_out_source" varchar,
	"subscription_management_confirmed_opt_out_reason" varchar,
	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
);
...
CREATE UNIQUE INDEX IF NOT EXISTS "contacts_personal_info_personal_info_email_address_idx" ON "contacts" ("personal_info_email_address");
...
DO $$ BEGIN
 ALTER TABLE "events_rels" ADD CONSTRAINT "events_rels_locations_id_locations_id_fk" FOREIGN KEY ("locations_id") REFERENCES "locations"("id") ON DELETE cascade ON UPDATE no action;
EXCEPTION
 WHEN duplicate_object THEN null;
END $$;
