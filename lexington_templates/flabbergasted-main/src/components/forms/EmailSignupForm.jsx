...
const EmailSignupForm = ({ buttonBgColor = 'bg-green-500' }) => {
  const [formSubmitted, setFormSubmitted] = useState(false);
  const [responseMessage, setResponseMessage] = useState('');
  const [showReward, setShowReward] = useState(false);
  const { reward, isAnimating } = useReward('rewardButton', 'emoji', {
    angle: 90,
    startVelocity: 35,
    elementCount: 70,
    spread: 100,
    ...
    elementSize: 18,
    zIndex: 1000,
    colors: ['#f5f5f5', '#c8bdfc', '#b7d4da', '#f3c4cd'], // black, white, lilac, and seafoam green colors
    // emojis: ['🎉'], // celebration emoji
  });
  ...
  const submit = async (event) => {
    ...
    const formattedData = {
      form: 1,
      id: 1, // Ensure this id is relevant to your backend logic
      submissionData: Object.entries(dataObject).map(([field, value]) => ({
        field,
    ...
    try {
      ...
      setShowReward(true);
      reward();
      // Use a short delay before updating component state to display the thank you message
      setTimeout(() => {
        setResponseMessage(responseData.message);
        setFormSubmitted(true);
      }, 2000); // Adjust this delay as needed, 500ms should be enough for the confetti to start
  ...
  return (
    <>
      {!formSubmitted ? (
        <form onSubmit={submit} className="bg-white border-2 border-black shadow-black overflow-hidden shadow justify-between mt-8 rounded-xl sm:flex">
          ...
          />
          <button
            className={`text-black flex items-center text-lg font-bold focus:outline-none justify-center text-center ${buttonBgColor} lg:border-l-2 border-black transform transition hover:shadow-none duration-200 hover:${buttonBgColor} w-full h-20 sm:w-20 shrink-0`}
        ...
        <div className="relative flex flex-col items-center justify-center bg-black pt-4 pb-5">
          {showReward && <span id="rewardOverlay" />} {/* Confetti will explode from here */}
          <div className="text-center pl-6 pr-6 max-w-sm mx-auto bg-black rounded-xl shadow-md flex items-center space-x-4">
            <div className="max-w-lg mx-auto mt-4 lg:text-xl tracking-wide text-secondary">
              🎉🎉🎉
              <div className="text-2xl font-medium text-white">Awesome, you're in!</div>
              <p className="text-lg font-medium text-white">Your 5% lifetime discount is locked in! Thanks so much for joining Bloom, we both have a lot to be excited about! Keep an eye out for a welcome email from the Bloom team and for upcoming emails about Bloom's launch! </p>
              {/* Response message */}
            </div>
          </div>
        </div>
