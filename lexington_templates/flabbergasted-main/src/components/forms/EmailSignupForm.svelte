<script lang="ts">
  ...
  {#if !formSubmitted}
  <form on:submit={submit}  class="bg-white border-2 border-black shadow-black overflow-hidden shadow justify-between mt-8 rounded-xl sm:flex">
    <label class="sr-only" for="email-address">Email address</label>
     <input
       autocomplete="email"
       class="w-full px-5 text-center lg:text-left border-transparent focus:ring-black h-20 text-sm py-3 bg-transparent focus:border-black placeholder-black rounded-xl"
       id="email-address"
       name="email"
     ...
     />
     <button
       class="text-black flex items-center text-lg font-bold focus:outline-none justify-center text-center bg-green-500 lg:border-l-2 border-black transform transition hover:shadow-none duration-200 hover:bg-green-600 w-full h-20 sm:w-20 shrink-0"
       type="submit"
     >
       <svg
         class="w-10 h-10"
